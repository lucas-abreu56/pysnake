...
def game_loop(window, game_speed):
    ...
    # Base para movimentação do personagem
    while True:
...
def finish_game(score, window):
    ...
    finish_message = f'Você perdeu! Você coletou o total de {score} frutas!'
    y = int(height / 2)
    x = int((width - len(finish_message)) / 2)
    window.addstr(y, x, finish_message)
    window.refresh()
    time.sleep(3)
...
def move_actor(actor, direction):
    match direction:
            ...
            case curses.KEY_DOWN:
                actor[0] += 1
            case _: # Não apertou tecla ou fora das setinhas
                pass
...
def select_difficulty():
    ...
    while True:
        ...
        if game_speed is not None:
            return game_speed
        print('Valor incorreto! Escolha a dificuldade de 1 a 5: ')

if __name__ == '__main__':
    # Criação de uma tela temporária no terminal. Após fechar, volta ao normal
